...
model = st.selectbox("Select Recommender Model", [
    'meta', 'weighted', 'switching', 'feature', 'svd', 'cbf', 'mixed', 'cascade'
])
top_n = st.slider("Top N Recommendations", 1, 20, 5)
alpha = st.slider("Alpha (Weighting Factor for Weighted Hybrid)", 0.0, 1.0, 0.5) if model == 'weighted' else None

if st.button("Get Recommendations"):
    with st.spinner("Fetching recommendations..."):
        try:
            recs = recommend(user_id, model=model, top_n=top_n, alpha=alpha or 0.5, verbose=False)
