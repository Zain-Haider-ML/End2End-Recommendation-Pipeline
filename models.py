...
movies['genres'] = movies[genre_cols].apply(lambda row: ' '.join([col for col in genre_cols if row[col] == 1]), axis=1)
...
def build_meta_model():
    ...
    for _, row in ratings.iterrows():
        ...
        idx = movie_indices.get(movie_id, None)
        content_feat = tfidf_matrix[idx].toarray()[0] if idx is not None else np.zeros(tfidf_matrix.shape[1])
